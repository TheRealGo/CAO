...
CAO manager window には左側の細い dashboard pane が自動で維持され、監視中の worker、推定状態(`work`, `ready`, `block`, `idle`, `miss`)、登録済み外部 target の一覧を表示します。worker の追加・削除・消失は動的に反映されます。

## CAO が内部でやること

`XXX で YYY を実装して` と言われたら、CAO は:

1. 現在の CAO tmux session を作成または再利用、
2. CAO manager window の supervisor dashboard を表示、
3. `XXX` 用のウィンドウを作成、
...
`cao send` は送信キー(`claude` には `C-m` / Enter、`codex` には `M-Enter` / Option+Return)をウィンドウに記録された runner から自動で選びます。

`cao add` で作られていない既存 tmux ウィンドウは、`cao send` で入力を送る前に明示的な runner 付きで登録する必要があります。登録済みの外部 window は `cao list` と引数なしの `cao capture` に含まれ、登録名や session prefix を後続の capture/send 対象解決に使えます。監督が終わったら登録解除します。
...
| 変数 | デフォルト | 意味 |
|---|---|---|
| `CAO_SESSION` | 現在の tmux session または `CAO` | tmux session 名 |
...
| `CAO_DASHBOARD_WIDTH` | `34` | 自動 manager dashboard pane の幅 |
...
同じ project config で CAO の強いローカル実行権限と runner binary 名も固定しています。session と worker のデフォルト runner は実際に動いている supervisor 環境から検出するため、今使っている tmux cockpit に追従します。
